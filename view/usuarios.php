...
$sql = "select * from user WHERE " . $campo . " like concat ('%',?,'%')"; //comando sql a ser mandado para o banco
...
<body>
  ...
  <div class=" container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
      ...
      <form class="d-flex" role="search" action="usuarios.php" method="get">
  ...
  <form action="../usuario/excluirusuarios.php" method="post">
    <table class="table table-dark table-striped">
      <thead>
        <tr>
          <th> <input class="btn btn-outline-success" type="submit" value="APAGAR" title="Apagar"> </th>
          <th scope="col"><a href="?campo=user.id"> ID </a></th>
          <th scope="col"><a href="?campo=user.nome">NOME</a></th>
          <th scope="col"><a href="?campo=user.usuario">USUARIO </a></th>
          <th scope="col"><a href="?campo=user.senha">SENHA</a></th>
          <th> &nbsp; </th>
          <th> &nbsp; </th>
      ...
      <tbody>
        ...
        ?>
          <tr>
            <td><input type="checkbox" name="ids[]" value="<?= $linha['id'] ?>"></td>
            <td><?= $linha['id'] ?></td>
            <td><?= $linha['nome'] ?></td>
            <td><?= $linha['usuario'] ?></td>
            <td><?= $linha['senha'] ?></td>
            <td> <a href="formusuario.php?id=<?= $linha['id'] ?>"> Editar </a> </td>
            <td> <a href="../usuario/excluiusuario.php?id=<?= $linha['id'] ?>"> Excluir </a> </td>
          </tr>
        <?php
        }
        ?>
